...
for _, arg in ipairs(arg) do
  ...
  elseif arg == "--coverage" then
    coverage = true
  elseif arg:find("^--seed=([a-z0-9]+)$") then
    _ENV.mcfly_seed = tonumber(arg:match("^--seed=([a-z0-9]+)$"), 16)
  elseif root_dir == nil then
    root_dir = arg
  else
    io.stderr:write(("Unknown arg %q\n"):format(arg))
    error()
  end
end

root_dir = shell.resolve(root_dir or "spec")
...
-- Ensure the test and src folders are also on the package path
do
  local function extend_path(path)
    package.path = ("%s;/%s/?.lua;/%s/?/init.lua"):format(package.path, path, path)
  end
  extend_path(root_dir)

  local src_dir = fs.combine(root_dir, "../src")
  if fs.isDir(src_dir) then extend_path(src_dir) end
end

do
  _ENV.describe, _ENV.it, _ENV.pending, _ENV.expect, _ENV.stub, _ENV.fail = describe, it, pending, expect, stub, fail
